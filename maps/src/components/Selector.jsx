      ...
      <div className="w-full md:px-20 lg:px-30 px-10">
        <div className="md:flex justify-between">
          <div className="md:w-1/2 w-full">
            ...
            {waypoints.length > 0 &&
            <>
              <p className="text-gray-500 font-semibold">Stops</p>
              { waypoints.map((waypoint, index) => {
                  return (
                    <p key={index} className="text-sm text-gray-900 flex"> <BsFillStopFill className="my-auto" /> {waypoint.location}</p>
                  )})}
            </>
            }
            <InputField title="Stop" val={stopRef}/>
            <p onClick={handleAddStop} className="-my-3 flex justify-end text-sm cursor-pointer select-none"> <AiOutlinePlusCircle className="my-auto mr-2" /> {waypoints.length>0?'Add another stop':'Add stop'}</p>
